...
import {
  createRequest, deleteRequest, getCapdevById, getRequestFieldDefinitions, getRequestStatusUpdates, getRequestsByCapdev, updateRequest, uploadFilesToGoogleDrive, type StatusAttachment,
} from '@/app/actions';
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 72px)' }}>
      ...
      <Dialog open={editorOpen} onClose={() => !saving && setEditorOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogActions sx={{ p: 2.5 }}><Button onClick={() => setEditorOpen(false)} disabled={saving} color="inherit">Cancel</Button><Button onClick={saveRequest} disabled={saving || !form.description || !form.requestedBudget} variant="contained">{saving ? 'Saving' : 'Save Request'}</Button></DialogActions>
      </Dialog>
